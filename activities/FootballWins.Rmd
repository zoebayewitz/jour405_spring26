...
ANSWER HERE: The majority of this model is useful, with some caveats. A low p-score and F-statistic show clear statistical significance. The adjusted R-squared shows that the model accounts for 58% of variance, which is just over half of the data. However, the Yards Gained have a negative co-efficient and therefore doesn't make much sense-- therefore, it should be excluded from the model. Overall, the model is valid and can be used, but factors such as the Residual Standard Error and Yards Gained make the model less precise.
...
ANSWER HERE: A better model would be a linear regression where Yards Gained are dropped; it is correlated highly with Points Scored and doesn't add other useful information. Eliminating Yards Gains would eliminate redundancy and statistical insignificance. I would keep Points Scored and Takeaways, which both show significant relationships with wins, and would produce cleaner, more reliable coefficients. I would add Giveaways, which shows the turnover from one team to the other, and would have a strong correlation with Takeaways. 
...
This model can be used to evaluate if a team's wins correlate with their performance in games. Using a team's Points Scored, Takeaways, and Giveaways allows us to predict how many wins a team might have. This can be compared to their actual record to see if it's accurate. In journalism, this can be used to write either about a team's success and if it's sustainable, or a team's losses and if they're improving.
